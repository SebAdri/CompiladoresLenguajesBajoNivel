...
void escribirEnArchivo(string componenteLexico, FILE * archivo);
...
void escribirEnArchivo(string componenteLexico, FILE * archivo)
{
	if (!(archivo=fopen("output.txt","a")))
	{
		printf("Archivo no encontrado.\n");
	}
	else
	{
		fprintf(archivo," %s",componenteLexico);
	}
}
	...
	if(argc > 1)
	{
		...
		while (t.compLex!=EOF){
			if(t.componente)
			{
       			...
       			escribirEnArchivo(t.componente, archivo);
			}
			sigLex();
		}
		fclose(archivo);
